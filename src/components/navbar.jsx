...
const Navbar = ({ productsRef }) => {
  ...
  return (
    <nav
    className={`fixed top-0 left-0 w-full mx-auto flex items-center justify-between md:justify-between px-6 md:px-16 py-4 z-50 transition-all duration-300 
    ${isScrolled ? "shadow-lg bg-white text-black" : "shadow-none bg-transparent text-white"}`}
  >
    {/* Logo */}
    <span className={`text-lg font-bold ${isScrolled ? "text-black" : "text-white"}`}>
      ISSABUL ABILITY VENTURES
    </span>
          ...
          {link.id === "Products" ? (
            ...
            <Link
              ...
              className={`cursor-pointer transition-colors duration-300 flex items-center ${
                isScrolled ? "text-black hover:text-yellow-400" : "text-white hover:text-yellow-400"
              }`}
            >
              {link.name}
            </Link>
