...
const About = ({ aboutRef }) => {
  ...
  return (
    <motion.section
      ...
      initial="hidden"
      animate="visible" // Ensures animation triggers correctly
      viewport={{ once: true, amount: 0.2 }} 
      className="mx-auto max-w-6xl py-16 lg:py-24"
    >
      <div className="flex flex-col-reverse lg:flex-row-reverse items-center justify-between gap-8 lg:gap-14 md:items-center">
        {/* Image Section */}
        <motion.div
        ...
        <motion.div 
          ...
          custom={1}
        >
          <h2 className="text-4xl font-bold text-gray-800 mb-4">Meet Innovator</h2>
          ...
          >
            {impactData.map((item, index) => (
              <motion.div
                key={index}
                className={`flex items-center p-5 shadow-md rounded-lg bg-white transition-transform transform hover:scale-105 hover:shadow-lg hover:bg-yellow-50 
                  ${index === 2 ? "sm:col-span-2 justify-center" : ""}`}
                variants={fadeInVariant}
                custom={index} 
              >
                <GifIcon 
